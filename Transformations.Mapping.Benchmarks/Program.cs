...
{
    private BenchmarkUser[] sourceArray = Array.Empty<BenchmarkUser>();

    [GlobalSetup]
    public void Setup()
    {
        sourceArray = BenchmarkData.CreateUsers(1_000).ToArray();
    }

    [Benchmark(Baseline = true)]
    public BenchmarkUserDto[] ArrayFastPath()
    {
        return sourceArray.ToBenchmarkUserDtoArray();
    }

    [Benchmark]
    public BenchmarkUserDto[] MemoryMapping()
    {
        return new global::System.Memory<BenchmarkUser>(sourceArray).ToBenchmarkUserDtoArray();
    }

    [Benchmark]
    public BenchmarkUserDto[] ReadOnlyMemoryMapping()
    {
        return new global::System.ReadOnlyMemory<BenchmarkUser>(sourceArray).ToBenchmarkUserDtoArray();
    }
}
...
public class AsyncEnumerableMappingBenchmarks
{
    ...
    [Benchmark]
    public async global::System.Threading.Tasks.Task<int> AsyncEnumerableMapping()
    {
        int count = 0;
        await foreach (var item in source.ToList().ToAsyncEnumerable().ToBenchmarkUserDtoAsyncEnumerable())
        {
            count++;
        }
        return count;
...
{
    public static async global::System.Collections.Generic.IAsyncEnumerable<T> ToAsyncEnumerable<T>(this global::System.Collections.Generic.IEnumerable<T> source)
    {
        foreach (var item in source)
        {
            yield return await global::System.Threading.Tasks.Task.FromResult(item);
        }
    }
}
